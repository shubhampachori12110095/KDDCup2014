...
There are some of the questions posted by the KDD Cup challenge of this year. I tried to answer them with the help of some visualizations. I hope they are of help to all of you as they have been to me. Enjoy!
...
```{r results='hold', fig.width=11, fig.height=5}
seasonplot(myts) 
seasonplot(mytsAll) 
```
...
Here we can see the expected values during those months and the 80% (dark blue) and 95% (light blue) Confidence level for prediction intervals. The solid line is the mean.
...
```
Backcasting the data is a basically doing a forecast with reverted data, though the visualization process is a little tricky. It was done with the help of the article "Backcasting in R"
...
positiveFrequencies <- c(rev(as.numeric(bc$mean)), positiveFrequencies[,2], as.numeric(modelForecast$mean))
...
#Change zero chance to a positive yet low probability
